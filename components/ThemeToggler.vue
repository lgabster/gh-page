<template>
  <button class="rounded-full bg-base-300 w-8 h-8 shadow-lg transition-all" @click="toggleTheme">
    <svg
    ...
    >
      <mask id="mask">
        ...
        <transition name="moonMove" enter-active-class="moonrise" leave-active-class="moonset">
          <circle v-if="!isDark" cx="18" cy="6" r="8" fill="black" stroke-width="0" class="mask" />
        </transition>
      </mask>
      ...
      <transition name="sunFlaire" enter-active-class="sunrise" leave-active-class="sunset">
        <g class="sunFlaire" v-if="isDark" stroke="currentColor">
...
const toggleTheme = () => {
  mainStore.setTheme(isDark.value ? 'light' : 'dark')
}
</script>
